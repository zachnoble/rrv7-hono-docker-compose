	...
	describe('POST /auth/register', () => {
		it('should register a new user and send a verification email', async () => {
			...
			expect(sendEmailMock).toHaveBeenCalledWith({
				...
				html: expect.stringContaining(
					`${env.FRONTEND_URL}/verify-user?token=${verificationToken.token}&email=${data.email}`,
				),
			})
		})
		...
		it('should allow re-registration with existing unverified email', async () => {
			...
			expect(sendEmailMock).toHaveBeenCalledWith({
				...
				subject: 'Verify Your Email Address',
				html: expect.stringContaining(
					`${env.FRONTEND_URL}/verify-user?token=${verificationToken.token}&email=${unverifiedUser.email}`,
				),
			})
		})
	...
	describe('POST /auth/forgot-password', () => {
		it('should send password reset email for existing user', async () => {
			...
			expect(sendEmailMock).toHaveBeenCalledWith({
				...
				html: expect.stringContaining(
					`${env.FRONTEND_URL}/reset-password?token=${passwordResetToken.token}&email=${user.email}`,
				),
			})
		})
	...
	describe('POST /auth/verify-user', () => {
		...
		it('should not verify email with expired token', async () => {
			...
			expect(sendEmailMock).toHaveBeenCalledWith({
				...
				subject: 'Verify Your Email Address',
				html: expect.stringContaining(
					`${env.FRONTEND_URL}/verify-user?token=${newToken.token}&email=${user.email}`,
				),
			})
		})
	})
	...
	describe('E2E Auth Flow', () => {
		it('should register -> verify -> login -> forgot password -> reset password -> login -> change password -> login', async () => {
			...
			expect(sendEmailMock).toHaveBeenCalledWith({
				...
				subject: 'Verify Your Email Address',
				html: expect.stringContaining(
					`${env.FRONTEND_URL}/verify-user?token=${verificationToken.token}&email=${user.email}`,
				),
			})
