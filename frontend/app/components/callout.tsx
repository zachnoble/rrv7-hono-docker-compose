...
import { type TextProps, composeRenderProps } from 'react-aria-components'
...
export function CalloutIcon({ className, ...props }: React.JSX.IntrinsicElements['div']) {
	return (
		<div
			{...props}
...
export function CalloutHeading({
	...
	const { 'aria-labelledby': ariaLabelledby, inline } = React.useContext(CalloutContext)
...
export function CalloutDescription({ className, id, ...props }: Omit<TextProps, 'elementType'>) {
	const { inline, 'aria-describedby': ariaDescribedBy } = React.useContext(CalloutContext)
...
export function CalloutActions({ className, ...props }: React.JSX.IntrinsicElements['div']) {
