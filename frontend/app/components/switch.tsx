...
import { composeTailwindRenderProps, groupBox } from './fns'

export function SwitchGroup(props: GroupProps) {
	return (
		<LabeledGroup>
			<Group {...props} className={composeTailwindRenderProps(props.className, groupBox)} />
		</LabeledGroup>
	)
}
				...
				<>
					<div
						className={twMerge(
							...
							renderProps.isFocusVisible && 'outline-2 outline-ring outline-offset-2',
						)}
					>
						<span
							data-ui='handle'
							className={twMerge(
								...
								renderProps.isSelected && [
									'rtl:-translate-x-5 translate-x-5 bg-[lch(from_var(--color-accent)_calc((49.44_-_l)_*_infinity)_0_0)]',
									size !== 'lg' && 'sm:rtl:-translate-x-3 sm:translate-x-3',
								],
							)}
						/>
					</div>
