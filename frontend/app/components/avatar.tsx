...
import { type FallbackAvatarProps, getFallbackAvatarDataUrl } from './initials'
	...
	return (
		<AvatarContext.Provider value={{ badgeId }}>
			<div
				...
				role='img'
				className={twMerge([
					'group @container relative isolate flex size-10 shrink-0 ring-background',
					status === 'loaded' &&
						'dark:-outline-offset-1 dark:outline dark:outline-white/10',
...
export const AvatarBadge = ({ badge, ...props }: AvatarBadgeProps) => {
	...
	return (
		<span
			...
			id={context.badgeId}
			className={twMerge([
				'absolute end-0 bottom-0 grid place-items-center rounded-full bg-background bg-clip-content',
				props.className,
			])}
		>
			{badge}
...
export function AvatarGroup({ reverse = false, className, ...props }: AvatarGroupProps) {
	return (
		<div
			{...props}
			className={twMerge(
				'-space-x-2 isolate flex items-center rtl:space-x-reverse',
				'[&>[role=img]:not([class*=ring-4])]:ring-2',
				reverse && '[&>[role=img]:last-of-type]:-me-2 flex-row-reverse justify-end',
				className,
			)}
		/>
	)
