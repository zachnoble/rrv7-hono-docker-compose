...
export function TagGroup(props: AriaTagGroupProps) {
	return <AriaTagGroup {...props} className={twMerge('flex flex-col gap-1', props.className)} />
}
		...
		<AriaTag
			...
			className={composeRenderProps(
				...
				(className, { isFocusVisible, isDisabled }) => {
					return twMerge(
						'flex max-w-fit cursor-default items-center gap-x-1 rounded-md px-2 py-0.5 font-medium text-xs/5 outline-0 transition data-selection-mode:cursor-pointer',
						'data-[selected=true]:text-white dark:data-[selected=true]:text-white',
						colors[color],
						isFocusVisible && 'outline-2 outline-ring outline-offset-1',
