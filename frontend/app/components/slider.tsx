			...
			{({ state, orientation }) => {
				return (
					<>
						<div
							className={twMerge(
								'group-data-[orientation=vertical]:-translate-x-1/2 group-data-[orientation=vertical]:-translate-y-1/2 bg-zinc-200 dark:bg-zinc-600',
								trackStyle,
							)}
						/>
						<div
							...
							<SliderThumb
								...
								className={composeRenderProps(
									...
									(className, { isFocusVisible, isDragging, isDisabled }) =>
										twMerge(
											'size-4 rounded-full border-2 border-accent bg-[lch(from_var(--color-accent)_calc((49.44_-_l)_*_infinity)_0_0)] shadow-xl dark:border-3',
